...
int main(int argc,char *argv[]){
    // Parameters (1 ~ 5): players, cards, decks, round.
    int Parameters[4] = {4,5,2,1}; 
    ...
    for (int i = 1; i < argc; i++){   
        ...
        switch (*(argv[i] + 1)){
            ...
            case '-':
                ...
                break;
            case 'n': 
                Parameters[0] = atoi(argv[i + 1]) > 2? atoi(argv[i + 1]) : 4;  i++;
                break;
            case 'c': 
                Parameters[1] = atoi(argv[i + 1]) > 1? atoi(argv[i + 1]) : 5;  i++;
                break;
            case 'd': 
                Parameters[2] = atoi(argv[i + 1]) > 1? atoi(argv[i + 1]) : 2;  i++;
                break;
            case 'r':
                Parameters[3] = atoi(argv[i + 1]) > 0? atoi(argv[i + 1]) : 1;  i++;
                break;
        }
    }

    // Start!
    server(logname, Parameters);
}
